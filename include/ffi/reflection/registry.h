...
#ifndef LITETVM_FFI_REFLECTION_REGISTRY_H
#define LITETVM_FFI_REFLECTION_REGISTRY_H
...
protected:
    ...
    static int FieldSetter(void* field, const TVMFFIAny* value) {
        ...
        if constexpr (std::is_same_v<T, Any>) {
            *static_cast<T*>(field) = AnyView::CopyFromTVMFFIAny(*value);
        } else {
...
public:
   ...
   */
    ...
    TypeAttrDef& def(const char* name, Func&& func) {
        ...
        Function ffi_func = GetMethod(std::string(type_key_) + "." + name, std::forward<Func>(func));
...
#endif//LITETVM_FFI_REFLECTION_REGISTRY_H
