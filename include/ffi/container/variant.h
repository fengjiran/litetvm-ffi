...
#ifndef LITETVM_FFI_CONTAINER_VARIANT_H
#define LITETVM_FFI_CONTAINER_VARIANT_H
...
class VariantBase {
public:
    TVM_FFI_INLINE bool same_as(const VariantBase& other) const {
...
protected:
    ...
    TVM_FFI_INLINE void SetData(Any other_data) {
        data_ = std::move(other_data);
    }

    TVM_FFI_INLINE Any MoveToAny() && {
        return std::move(data_);
    }

    NODISCARD TVM_FFI_INLINE AnyView ToAnyView() const {
        return data_.operator AnyView();
    }
...
protected:
    ...
    TVM_FFI_INLINE void SetData(ObjectPtr<Object> other) {
        data_ = std::move(other);
    }

    TVM_FFI_INLINE Any MoveToAny() && {
        return {ObjectRef(std::move(data_))};
    }

    NODISCARD TVM_FFI_INLINE AnyView ToAnyView() const {
        TVMFFIAny any_data;
        if (data_ == nullptr) {
            any_data.type_index = kTVMFFINone;
            ...
            any_data.zero_padding = 0;
            any_data.v_obj = ObjectUnsafe::TVMFFIObjectPtrFromObjectPtr<Object>(data_);
        }
...
public:
    ...
    static_assert(details::all_storage_enabled_v<V...>, "All types used in Variant<...> must be compatible with Any");
   ...
   */
    ...
    Variant(const Variant& other) : TParent(other.data_) {}
    Variant(Variant&& other) noexcept : TParent(std::move(other.data_)) {}

    TVM_FFI_INLINE Variant& operator=(const Variant& other) {
        this->SetData(other.data_);
        return *this;
    }

    TVM_FFI_INLINE Variant& operator=(Variant&& other) noexcept {
   ...
   */
    ...
    TVM_FFI_INLINE std::string GetTypeKey() const {
        return this->TParent::ToAnyView().GetTypeKey();
    }

private:
    friend struct TypeTraits<Variant>;
   ...
   * ObjectRef
   */
    NODISCARD TVM_FFI_INLINE Object* GetObjectPtrForHashEqual() const {
        constexpr bool all_object_v = (std::is_base_of_v<ObjectRef, V> && ...);
        static_assert(all_object_v, "All types used in Variant<...> must be derived from ObjectRef "
...
template<typename... V>
struct TypeTraits<Variant<V...>> : TypeTraitsBase {
...
template<typename... V>
TVM_FFI_INLINE bool ObjectPtrEqual::operator()(const Variant<V...>& a, const Variant<V...>& b) const {
