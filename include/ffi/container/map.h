...
#ifndef LITETVM_FFI_CONTAINER_MAP_H
#define LITETVM_FFI_CONTAINER_MAP_H
...
private:
   ...
   */
    static void InsertMaybeReHash(KVType&& kv, ObjectPtr<Object>* map) {
        ...
        uint64_t next_size = std::max(map_node->NumSlots() * 2, static_cast<uint64_t>(kInitSize));
        next_size = std::min(next_size, static_cast<uint64_t>(kMaxSize));
...
private:
    ...
    static constexpr uint8_t kEmptySlot = 0b11111111;
    /*! \brief Binary representation of the metadata of a protected slot */
    static constexpr uint8_t kProtectedSlot = 0b11111110;
...
private:
    ...
    void Reset() {
        ...
        for (uint64_t bi = 0; bi < n_blocks; ++bi) {
            ...
            for (int j = 0; j < kBlockCap; ++j, ++meta_ptr, ++data_ptr) {
                ...
                if (meta != static_cast<uint8_t>(kProtectedSlot) && meta != static_cast<uint8_t>(kEmptySlot)) {
                    meta = static_cast<uint8_t>(kEmptySlot);
   ...
   */
    static ObjectPtr<DenseMapObj> Empty(uint32_t fib_shift, uint64_t n_slots) {
        ...
        for (uint64_t i = 0; i < n_blocks; ++i, ++block) {
            std::fill(block->bytes, block->bytes + kBlockCap, static_cast<uint8_t>(kEmptySlot));
        }
        return p;
    }
    /*!
    ...
    static ObjectPtr<DenseMapObj> CopyFrom(DenseMapObj* from) {
        ...
        for (uint64_t bi = 0; bi < n_blocks; ++bi) {
            ...
            for (int j = 0; j < kBlockCap;
                ...
                if (meta != static_cast<uint8_t>(kEmptySlot)) {
   ...
   */
    ...
    struct ListNode {
        ...
        bool IsEmpty() const { return Meta() == static_cast<uint8_t>(kEmptySlot); }
        /*! \brief If the entry is protected slot */
        bool IsProtected() const { return Meta() == static_cast<uint8_t>(kProtectedSlot); }
        /*! \brief Set the entry to be empty */
        void SetEmpty() const { Meta() = static_cast<uint8_t>(kEmptySlot); }
        ...
        void SetProtected() const { Meta() = static_cast<uint8_t>(kProtectedSlot); }
